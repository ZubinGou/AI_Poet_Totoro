...
if __name__ == '__main__':
    ...
    done_id1 = set()
    if os.path.exists('done_id1.txt'):
        print('正在从上次爬取数据中恢复...')
        done_file = open('done_id1.txt', 'r')
        done_id1 = done_file.read()
        done_id1 = eval(done_id1)
        done_file.close()
        print('之前已经爬取了{}个歌手的全部歌词了'.format(len(done_id1)))
    ...
    with open('all_singer.txt', 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if art_id not in done_id1:
                print(art_id)
                crawl_lyrics(gender, art_id)
                done_id1.add(art_id)
                done_file = open('done_id1.txt', 'w')
                done_file.write(str(done_id1))
                done_file.close()
                print('已经爬取了{}个歌手的全部歌词了'.format(len(done_id1)))
